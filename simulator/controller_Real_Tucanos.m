...
%   2/18/2014 - R. Beard
%   2/24/2014 - R. Beard
%   1/20/2015 - Team Tucanos
%   LAST EDIT: 9:08pm 1/20/2015

function v_c=controller_home_full_state(uu,P)

persistent ballpositionX;
persistent ballpositionY;
...
%if we haven't init'ed the persistent variables
if (t < 1)
    ballpositionX = ball(1);
    ballpositionY = ball(2);
    timeStart = t;
end

if (timeStart+2 < t) && (abs(ballpositionX-ball(1))< P.ball_radius*10) && (abs(ballpositionY-ball(2))< P.ball_radius*10)
        %reset values
        ballpositionX = ball(1);
        ballpositionY = ball(2);
        timeStart = t;
        %spin the robots
        v1 = skill_spin_ccw(robot(:,1), 0.5, P);
        v2 = skill_spin_cw(robot(:,2), 0.5, P); 
else
